...
4.4 & 
Heatmap generation & 
Generate a simple heatmap showing areas where pedestrians frequently appear or move through. The heatmap is used for visualization rather than precise scientific measurement. & 
Implemented \\
\hline
...
5.1 & 
Dashboard analytics & 
Display analysis results through a dashboard, including zone statistics, charts, heatmap, transition information, and alerts. & 
Implemented \\
\hline
...
& View processed video, KPI cards, crowded zone, popular path, system status, heatmap preview, and camera-specific analysis results. 
& Administrator, Analyst, Security Staff \\
\hline
...
& View zone counts, line-zone statistics, movement timeline, heatmap, transition flow, and activity information. 
...
\begin{enumerate}
    ...
    \item When processing finishes, the dashboard is updated with processed video, total people, heatmap, and available analytics.
\end{enumerate} \\
\hline
Alternative flow &
\begin{enumerate}
...
Actor & Administrator, Analyst, Security Staff \\
\hline
Description & Allows users to view processed video, KPI cards, crowded zone, popular path, system status, heatmap preview, and camera-specific analysis results. \\
\hline
...
\begin{enumerate}
    ...
    \item The system displays processed video and heatmap preview when available.
...
\begin{enumerate}
    ...
    \item For each camera with a completed job, the system displays preview, total people, peak zone, popular path, and alert count.
...
\hline
Description & Allows users to view polygon-zone counts, line-zone statistics, movement timeline, heatmap, transition flow, alerts, and activity information for completed analysis jobs. \\
\hline
...
\begin{itemize}
    \item The interface shall be understandable for non-technical administrators and operators.
...
\hline
UserManagementPage & Allows the administrator to create, view, update, and deactivate user accounts stored in SQLite. \\
\hline
...
video. \\
\hline
DashboardPage & Displays the main analysis results, including KPI cards, processed video, realtime video-window statistics, zone count charts, transition flow, heatmap, and congestion status. \\
\hline
...
\hline
HeatmapPage & Displays the generated heatmap image for a completed analysis job. \\
\hline
...
\hline
ReportExportPage & Allows administrators and analysts to export completed analysis results as CSV, printable HTML, or backend-generated PDF reports. \\
\hline
\end{longtable}
...
Instead, it reads already-computed analytics such as zone counts, transition flow, density scores, dwell times, alerts, processed video paths, preview image paths, and heatmap paths.
...
\hline
HeatmapOutput & Represents the generated heatmap image for an analysis job. It is implemented as the heatmap path stored in the analysis job record. \\
\hline
...
The processing pipeline then generates a preview image, runs YOLO-based pedestrian detection, assigns temporary anonymous track IDs using ByteTrack-based tracking, computes analytics, generates a heatmap, and stores the resulting media paths.
...
A heatmap image is also generated as a visualization of frequent pedestrian presence and movement.
...
The dashboard retrieves stored analytics results from the backend, including total people, most crowded zone, popular path, zone counts, transition flow, heatmap path, processed video path, preview image path, density scores, dwell times, and congestion status.
...
The multi-camera overview shows all logical camera sources with the latest completed analysis result for each camera.
...
Authentication module & Handles login, session token creation, password verification, logout, and role-based access control for \texttt{admin}, \texttt{operator}, and \texttt{analyst}. \\
\hline
...
Dashboard and visualization module & Displays KPI cards, processed video, camera overview, zone statistics, heatmap, movement timeline, transition flow, alerts, and job results. \\
\hline
...
Storage module & Stores users, cameras, zones, jobs, analytics results, alerts, and report metadata in PostgreSQL, while videos, previews, heatmaps, and reports are stored as local files. \\
\hline
...
\begin{itemize}
    ...
    \item \textbf{style.css}: defines the visual layout and responsive design of the dashboard, including sidebar, topbar, cards, panels, forms, tables, video preview area, heatmap display, zone editor, and modal components.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \texttt{VideoProcessingService.js}: coordinates the video processing workflow by calling Python scripts, generating preview/video/heatmap outputs, reading trajectory data, saving analysis jobs, alerts, zone counts, flow records, and trajectory points to PostgreSQL.
    ...
    \item \textbf{backend/public/media/jobs}: stores generated media for each analysis job, including preview images, processed videos, heatmaps, trajectory CSV files, and statistics JSON files.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{heatmap.py}: generates a heatmap image from pedestrian trajectory points.
    ...
    \item \textbf{Runtime outputs}: each processing job can generate a preview image, processed video, heatmap image, trajectory CSV file, and statistics JSON file. These outputs are stored under \texttt{backend/public/media/jobs/<jobId>} and referenced by records in PostgreSQL.
\end{itemize}
...
\hline
Dashboard & Displays KPI cards, processed video, heatmap preview, congestion status, and selected job results. \\
\hline
...
\hline
9 & Heatmap visualization & Implemented & \path{ai_services/heatmap.py} \\
\hline
...
\section{Testing Implemented Functions}

Testing technique: API smoke testing, black-box testing, and functional testing.

% Suggested techniques include black-box testing, functional testing, equivalence partitioning, boundary value analysis, integration testing, and acceptance testing.

Testing environment: Localhost with Node.js, Python 3 (ultralytics, opencv), PostgreSQL, and a modern web browser.
...
\hline
Description & Verify that the system prevents access and displays an error when a user enters a valid email but an incorrect password. \\
\hline
...
Concluding remark: The system successfully authenticates users via the authService and prevents unauthorized access when invalid credentials are provided..
...
\hline
2 & Select unsupported file (e.g., .txt or .pdf) & Test validation & File is selected in input & Selected \\
\hline
3 & Click Upload & Submit invalid input & System displays unsupported file error; file is not saved & Blocked \\
\hline
\end{longtable}

Concluding remark: The backend middleware (multer) accurately filters file MIME types and safely aborts the upload process for non-video files without crashing.
...
\hline
Description & Verify that the system prevents or warns the user before running zone-based analytics without zone definitions. \\
\hline
Approach & Business logic testing -- exception case \\
\hline
\end{longtable}
...
\textbf{Step} & \textbf{Action} & \textbf{Purpose} & \textbf{Expected result} & \textbf{Comment} \\
\hline
1 & Upload/select a valid video & Prepare analysis input & Video is available & Ready \\
\hline
2 & Ensure no zone is configured & Create exception condition & Zone list is empty & Verified \\
\hline
3 & Click Start Analysis & Test business rule & System asks user to configure zones first or marks zone analytics unavailable according to confirmed policy: Proceeds by processing global counts and ignores zone-specific logic. & Handled safely \\
\hline
\end{longtable}

Concluding remark: The AI pipeline safely handles empty zone configurations (via zones.json returning empty lists) by still tracking total people accurately while bypassing local zone analytics.
...
\hline
3 & Inspect transition result & Verify analytics & Flow A $\rightarrow$ B increases when a track moves from A to B & Correct \\
\hline
\end{longtable}

Concluding remark: The tracking script successfully determines point-in-polygon status, and the backend analytics accurately groups consecutive unique zones to populate the transition flow records.
...
\hline
Description & Verify that the system creates an alert when zone count/density exceeds the configured threshold. \\
\hline
...
\textbf{Step} & \textbf{Action} & \textbf{Purpose} & \textbf{Expected result} & \textbf{Comment} \\
\hline
1 & Configure threshold for Zone A & Prepare alert condition & Threshold is saved (e.g., 10) & Configured \\
\hline
2 & Run or load analysis result with count above threshold & Trigger alert logic & Actual value exceeds threshold & Exceeded \\
\hline
3 & Open alert page & Verify result & New alert is displayed with zone, time, threshold, and actual value & Triggered \\
\hline
\end{longtable}

Concluding remark: The backend saveStatsToJob function correctly evaluates specific zone thresholds against actual counts and automatically inserts warning/danger records into the alerts table.
...
\hline
Usability & Ask user to complete login, upload, configure zones, and view dashboard & User can complete core tasks without detailed technical instruction & Passed \\
\hline
Performance & Measure processing time for confirmed demo video: sample\_data/mall.mp4 (720p) & Processing time is within confirmed target: Tens of seconds & Passed \\
\hline
Reliability & Test invalid video, missing zones, empty detection, failed job & System shows clear error/status and does not crash & Passed \\
\hline
Security & Attempt to access protected pages without login & System redirects or denies access & Passed \\
\hline
Privacy & Inspect stored pedestrian data & Only anonymous track IDs and analytics are stored & Passed \\
\hline
...
The confirmed scope of the final prototype is a local offline-video pedestrian flow monitoring system. The system allows users to register logical camera sources, upload short MP4 videos, configure polygon or line zones, run YOLOv8-based pedestrian detection and tracking, generate heatmaps and analytics, review alerts, and export reports.
...
\hline
Storage & At least 2 GB free space for source code, dependencies, uploaded videos, processed videos, heatmaps, and reports. \\
\hline
...
The computer vision processing pipeline is implemented in Python. It uses OpenCV for video processing and Ultralytics YOLOv8 for pedestrian detection. The pipeline includes preview extraction, detection, tracking, analytics computation, and heatmap generation.
...
\begin{enumerate}
    ...
    \item View the generated heatmap preview.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item Review the registered camera sources.
