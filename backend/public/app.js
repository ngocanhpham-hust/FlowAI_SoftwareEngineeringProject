...
async function loadRealtimeStats() {
    ...
    if (!auth.token || !hasSelectedCamera() || !video || video.paused || !video.currentTime) {
        return;
    }
...
async function uploadVideo() {
    ...
    resetZoneEditor("No zones configured for this camera");
    ...
    try {
        ...
        latestStatsSignature = "";
        await Promise.all([loadStats(), loadAlerts(), loadJobs(), loadFlow(), loadZonesForSelectedCamera(), loadMulticameraOverview()]);
...
function editCamera(cameraId) {
    ...
    editingCameraVideoId = String(cameraId);
    selectedJobId = "";
    input.value = "";
    input.click();
    handleCameraChange();
...
async function uploadEditedCameraVideo(event) {
    if (!canWrite()) return;

    const input = event.target;
    const file = input.files[0];
    const cameraId = editingCameraVideoId || selectedCameraId();

    if (!file || !cameraId) {
        return;
    }

    const uploadButton = document.querySelector(`button[onclick="editCamera(${cameraId})"]`);
    const originalText = uploadButton?.innerText || "Edit";
    const formData = new FormData();
    formData.append("video", file);
    formData.append("cameraSourceId", cameraId);
    ...
    resetZoneEditor("No zones configured for this camera");
    addFeed("Uploading replacement video...");
    ...
    try {
        ...
        latestStatsSignature = "";
        addFeed("Replacement video processed");
        await Promise.all([
            loadStats(),
            loadAlerts(),
            loadJobs(),
        ...
        alert("Video updated for this camera");
    } catch (err) {
        console.error(err);
        addFeed(err.message);
        alert(err.message);
...
async function deleteCamera(cameraId) {
    ...
    if (selectedCameraId() === String(cameraId)) {
        const select = document.getElementById("cameraSelect");
        select.value = [...select.options].find(option => option.value !== String(cameraId))?.value || "";
        selectedJobId = "";
    }
...
async function loadAlerts() {
    ...
    if (!hasSelectedCamera()) {
        if (list) list.innerHTML = `<div class="feed-item">No active alerts</div>`;
        if (managementList) {
...
async function loadFlow() {
    ...
    if (!hasSelectedCamera()) {
        if (list) list.innerHTML = `<div class="feed-item">No transition flow data</div>`;
        return;
    }
...
function resetZoneEditor(emptyMessage = "No zones configured for this camera") {
    zones = [];
    activeZoneIndex = 0;
    polygonMode = false;
    ...
    if (zoneList) {
        zoneList.innerHTML = `<div class="feed-item">${emptyMessage}</div>`;
    }
...
async function loadZonesForSelectedCamera() {
    ...
    if (!cameraId) {
        resetZoneEditor("Select a camera to configure zones");
...
function updateZoneThreshold(index, value) {
    ...
    if (!zones[index] || !Number.isFinite(threshold) || threshold < 1) return;
            ...
            <div class="zone-item-header">
                ...
                <label>
                    ...
                    <input
                        ...
                        min="1"
                        step="1"
                        value="${Number(zone.threshold) || 10}"
                        onchange="updateZoneThreshold(${index}, this.value)"
                        oninput="updateZoneThreshold(${index}, this.value)"
                    >
                </label>
            </div>
            <div class="row-actions">
